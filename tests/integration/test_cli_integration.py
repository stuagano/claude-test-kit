...
from ctk import Artifact, VerificationError, claim_vs_reality, expect, run, verify
    ...
    with pytest.raises(VerificationError, match="SILENT FAILURE"):
        claim_vs_reality(
            ...
            verifier=lambda: verify(
                Artifact(outfile, min_bytes=2, is_json=True, newer_than=run_started_at)
            ),
